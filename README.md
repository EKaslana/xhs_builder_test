...
| 平台 | 操作 | 持久化 | 说明 |
| --- | --- | --- | --- |
| **Render**（推荐） | 点击上面的按钮 | 免费版文件系统临时（重启重置，对演示反而是干净环境）；付费版可在 Dashboard 走 Settings → Disks 手动挂 1 GB 磁盘到 `/var/data` 即可持久化 | 仓库根 `render.yaml` 已配置好构建/启动/健康检查/9 个 LLM 环境变量占位 |
